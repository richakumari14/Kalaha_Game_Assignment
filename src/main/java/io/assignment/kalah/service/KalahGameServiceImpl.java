...
import io.assignment.kalah.repository.KalahGameRepository;
import lombok.extern.slf4j.Slf4j;

/**
 * Implementation of {@link KalahGameService}
 * 
 * @author Richa
 *
 */
...
public class KalahGameServiceImpl implements KalahGameService {
	...
	/**
	 * {@code KalahGameRepository} instance
	 */
	...
	@Autowired
	public KalahGameServiceImpl(final KalahGameRepository repository, final Environment environment) {
	 ...
	 * {@inheritDoc}
	 * 
	 * @see io.assignment.kalah.service.KalahGameService#createGame()
	 */
	@Override
	public KalahGameDTO createGame() {
		final KalahGame kalahGame = repository.save(new KalahGame(pitStoneCount));
	 ...
	 * {@inheritDoc}
	 * 
	 * @see io.assignment.kalah.service.KalahGameService#makeMove(String, Integer)
	 */
	@Override
	public KalahGameDTO makeMove(final String gameId, final Integer pitId) {
		final KalahGame kalahGame = repository.findById(gameId);
	...
	private void resetBoard(final KalahGame kalahGame) {
		kalahGame.getBoard().getPits().parallelStream().filter(pit -> (Player.PLAYER_A.getHouseIndex() != pit.getId())
				&& (Player.PLAYER_B.getHouseIndex() != pit.getId())).forEach(pit -> pit.setStoneCount(0));
	}
	...
	private void lastEmptyPit(final KalahGame kalahGame, final int endPitId) {
		...
		if (!endPit.isHouse() && endPit.getPlayerTurn().equals(kalahGame.getTurn()) && (endPit.getStoneCount() == 1)) {
	...
	private void validateMove(final KalahGame kalahGame, final int startPitId) {
		...
		if (Player.PLAYER_A.equals(kalahGame.getTurn()) && !Player.PLAYER_A.equals(startPit.getPlayerTurn())) {
			log.error("It's Player A turn");
			throw new IllegalMoveException("It's Player A turn");
		}
		if (Player.PLAYER_B.equals(kalahGame.getTurn()) && !Player.PLAYER_B.equals(startPit.getPlayerTurn())) {
